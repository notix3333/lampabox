...
test('configuredApiBaseUrl migrates the legacy production hostname', () => {
  const previousLampa = global.Lampa
  const stored = Object.create(null)
  global.Lampa = {
    Storage: {
      get: () => 'https://lampa-letterboxd-watchlist.rexikplay3.workers.dev/',
      set: (name, value) => {
        stored[name] = value
      },
    },
  ...
  assert.equal(
    configuredApiBaseUrl(),
    'https://lampa-letterboxd-watchlist.lampabox.workers.dev',
  )
  assert.equal(
    stored.letterboxd_watchlist_api_url,
    'https://lampa-letterboxd-watchlist.lampabox.workers.dev',
  )

  if (previousLampa === undefined) delete global.Lampa
  else global.Lampa = previousLampa
})
...
test('preset catalog validates remote data and falls back to built-ins', () => {
  assert.deepEqual(
    normalizePresetCatalog({
      version: 1,
      lists: [
        { id: 'valid', title: 'Valid', username: 'official', slug: 'valid-list' },
        { id: 'unsafe', title: 'Unsafe', username: '../bad', slug: 'unsafe' },
      ],
    }),
    [
      {
  ...
  assert.ok(normalizePresetCatalog(null).length >= 5)
})

test('preset selection adds, deduplicates, and removes canonical list paths', () => {
  const preset = { username: 'official', slug: 'letterboxds-top-500-films' }
  const added = togglePresetList('alice/list/favorites', preset, 'alice')
  assert.equal(added.added, true)
  assert.equal(
    added.value,
    'alice/list/favorites\nofficial/list/letterboxds-top-500-films',
  )

  const removed = togglePresetList(added.value, preset, 'alice')
  assert.equal(removed.added, false)
  assert.equal(removed.value, 'alice/list/favorites')
})
...
test('watched cache polling respects both cron cadence and Worker backoff', () => {
  const now = Date.parse('2026-09-06T00:00:00.000Z')

  assert.equal(watchedPollDelay(null, now), 315000)
  assert.equal(
    watchedPollDelay({ nextAttemptAt: '2026-09-06T01:00:00.000Z' }, now),
    3630000,
  )
})
...
test('Lampa integration registers settings and loads the Worker only once per session', async () => {
  ...
  const presetButton = params.find((param) => param.param.type === 'button')
  presetButton.onChange()
  await new Promise((resolve) => setImmediate(resolve))
  assert.equal(requestUrls.includes('https://worker.example/presets'), true)
  assert.equal(selections[1].items[0].title, 'Top 500')
  stored.letterboxd_public_lists = 'alice/list/favorites'
  selections[1].onSelect(selections[1].items[0])
  assert.equal(
    stored.letterboxd_public_lists,
    'alice/list/favorites\nofficial/list/letterboxds-top-500-films',
  )
