...
const USER_AGENT =
  'Mozilla/5.0 (compatible; LampaLetterboxdWatchlist/1.1; +https://github.com/notix3333/lampabox)'
...
async function fetchCollectionPage(
  ...
  if (isPrivatePage(html, target)) {
    const code = target.kind === 'watchlist' ? 'WATCHLIST_UNAVAILABLE' : 'LIST_UNAVAILABLE'
    const message =
...
async function fetchCollection(
  ...
  for (let page = 1; page <= MAX_LETTERBOXD_PAGES; page += 1) {
    ...
    const pageFilms =
      target.kind === 'watchlist' ? parseWatchlistPage(html) : parseListPage(html)
